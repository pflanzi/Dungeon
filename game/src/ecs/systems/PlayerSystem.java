...
public class PlayerSystem extends ECS_System {
                                ...
                                if (slot >= 0) {
                                    if (slot < ((InventoryComponent) ic).filledSlots()) {
                                        ...
                                        ((InventoryComponent) ic)
                                                .printInventory(inventoryItemSlotCounter);
                                    }
                                } else {
                                    inventoryItemSlotCounter =
                                            ((InventoryComponent) ic).filledSlots() - 1;
                                    ((InventoryComponent) ic)
                                            .printInventory(inventoryItemSlotCounter);
                                }
                            });
    ...
    private void accessInventoryUseActiveItem(Entity e, int slot) {
        e.getComponent(InventoryComponent.class)
                .ifPresent(
                        ic -> {
                            if (((InventoryComponent) ic).getItems().get(slot).getItemType()
                                    == ItemType.Active) {
                                ((InventoryComponent) ic).getItems().get(slot).triggerUse(e);
                            } else if (((InventoryComponent) ic)
                                            .getItems()
                                            .get(slot)
                                            .getItemCategory()
                                    == ItemCategory.BAG) {
                ...
                .ifPresent(
                        ic -> {
                            if (slot >= 0 && slot < ((InventoryComponent) ic).filledSlots()) {
                                ((InventoryComponent) ic)
                                        .removeItem(((InventoryComponent) ic).getItems().get(slot));

                                InventoryComponent inventoryComponent =
                                        entity.getComponent(InventoryComponent.class)
                                                .map(InventoryComponent.class::cast)
                                                .orElseThrow(() -> missingVC());
                ...
                .ifPresent(
                        ic -> {
                            if (itemSlot >= 0) {
                                ...
                                bagItemSlotCounter =
                                        ((InventoryComponent) ic).filledBagSlots(bagSlot) - 1;
                                ((InventoryComponent) ic).printBagInventory(bagSlot, itemSlot);
                            }
                        });
    }
